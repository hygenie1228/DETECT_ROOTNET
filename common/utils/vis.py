...
class Vis:
    ...
    def visualize_skeleton(img, keypoints, kps_lines, output_path):
        img = (img*255).cpu().numpy().transpose(1, 2, 0)
        new_img = img.copy()
        ...
        for i in range(len(keypoints)):
            ...
            cv2.circle(new_img, p, radius=3, color=colors[i], thickness=2)
        ...
        for i in range(len(kps_lines)):
            ...
            cv2.line(new_img, p1, p2, color=colors[i], thickness=2, lineType=cv2.LINE_AA)
